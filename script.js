...
window.addEventListener('DOMContentLoaded', () => {
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('#timer', '2022-07-01');
...
const images = () => {
  ...
  workSection.addEventListener('click', e => {
    e.preventDefault();
    let target = e.target; //проверка на target = проверка на кликабельность
    ...
    if (target && target.matches('div.popup')) {
      // проверка что кликнута div с классом popup
...
const tabs = (headerSelector, tabSelector, contentSelector, activeClass, display = 'block') => {
  const header = document.querySelector(headerSelector),
        ...
        clazz = document.querySelector(activeClass);
...
const timer = (id, deadline) => {
  ...
  function getTimeRemainding(endtime) {
    const total = Date.parse(endtime) - Date.parse(new Date()),
          // Возвратит количество оставшегося времени акции в миллисекундах.
